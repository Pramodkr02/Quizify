...
const processQuizData = (data) => {
  ...
  if (data.response_code !== 0) {
    ...
    const errorMessage = errorMessages[data.response_code] || `API error! Response code: ${data.response_code}`;
...
export const FALLBACK_QUIZ_DATA = {
  ...
  results: [
    ...
    },
    {
      ...
      difficulty: "medium",
      category: "History",
      question: "The United States declared independence from Great Britain on July 4th, 1776.",
      correct_answer: "True",
    ...
    },
    {
      ...
      incorrect_answers: ["300,000,000 meters per second", "299,000,000 meters per second", "301,000,000 meters per second"]
    },
    {
      type: "multiple",
      difficulty: "easy",
...
export const fetchQuizQuestionsWithFallback = async (options = {}) => {
  ...
  try {
    ...
    const fallbackData = {
      ...FALLBACK_QUIZ_DATA,
      results: FALLBACK_QUIZ_DATA.results.slice(0, Math.min(amount, FALLBACK_QUIZ_DATA.results.length))
    };
...
export const fetchQuizQuestionsSmart = async (options = {}) => {
  ...
  if (requestCount >= 3 && timeSinceLastRequest < 30000) { // 30 seconds
